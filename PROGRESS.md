...
- [~] Asset tint audit — every map-editor model gets the multi-colour treatment, not just bare geometry: `ModelFit.TintPalette` now also tints a surface whose albedo texture is missing or effectively white (the leftover-white-models cause), keeping only genuinely-coloured textures. Adds `ModelFit.WhiteRenderingSurfaces` + a headless `--audit-assets` sweep that runs the real load+tint pipeline over the whole ~2900-model library and prints every still-white model, plus synthetic regression tests. Full local sweep + owner eyeball pending (`godot` is locally gated)
- [ ] Sound effects — audio service (pooled players) + event-wired SFX (fire/explosion/wall-break/pickup/victory/UI) + volume setting; owner-chosen next, blocked on installing Stable Audio Open into the local ComfyUI (pipeline ready in the `Asset-Creation` repo)
