...
- [~] Web (WASM) export + Lundrea Arcade deploy (2026-06-27) — single-player is **LIVE** at <https://lundrea-arcade.web.app/tank/index.html> (experimental Godot .NET WASM via the ComplexRobot fork; vendored bundle in the ProjectX arcade). Fixed 5 web-only bugs (wasm-tools workload, trimmer pruning the game, dropped raw resources, `Guid.NewGuid()` collision breaking all combat, LFS deploy quota) + owner asks (quieter pew shot SFX, Exit→arcade, enemy name labels on, kill-streak = kills-since-death, real game card). Lives on branch `feat/web-export` (web-only `#if GODOT_WEB` changes, **not merged to main**). **NEXT: 8-player multiplayer** — menu → Solo/Multiplayer/Settings/Back-to-Arcade; Multiplayer → FFA or Team vs Team → 8-slot lobby + lobby browser → 3·2·1 countdown → arena; on the existing `tankgame-worker` Cloudflare Worker (Durable Object). **Full toolchain + resume guide + multiplayer plan: `docs/web-export.md`.** Desktop/main still needs the sound/name/streak fixes ported (SFX part = PR #229).
