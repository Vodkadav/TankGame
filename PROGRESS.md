...
- [~] Arcade-bug + multiplayer-lobby arc 2026-07-05 (`docs/multiplayer-lobby-plan.md`) — **flaky Solo damage fixed**: fixed-timestep sim (`FixedTimestep`, 1/60 s accumulator in `Arena3DScene`/`ArenaScene`) + swept segment-vs-circle projectile↔tank hits (`Projectile.PreviousPosition`, path-ordered) so laggy frames can no longer tunnel shots through tanks, plus zero-allocation `World.Step`/`CombatResolver` hot loop (WASM GC-hitch lag). Remaining: §6c menu-button audit + web keyboard seam; lobby Phases 0–9 (browser + 4-slot room + AI fill); renderer profiling on iPad; P8 re-export to ship.
