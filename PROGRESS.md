...
- [~] Editor overhaul batch 2 (owner follow-ups 2026-06-11, #200–#205) — resize persists content; isometric camera + orbit/pan; fly-out menus; bottom bar fix; unified ringed spawn markers; random spawn assignment + random respawns; selection gizmo (3 axis rings + size bar, free poses in format); asset browser foundation (decorations format, library catalogue + survey). REMAINING: asset browser part 2 — side-panel UI (search + categories), copy-on-place import into res:// with credits, decoration rendering in editor + match
