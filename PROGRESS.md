...
- [x] Audio + menu UX overhaul (owner asks 2026-06-18) — per-powerup pickup cues (9 kinds), distance-culled tank fire (~7 cells), cartoonier fire + punchier death boom, kill + streak voice callouts ("Enemy destroyed" / "Double/Triple/Multi kill", Kokoro TTS `bm_george`), quieter menu ping, name-prompt Cancel + settings Escape/click-out, the missing settings i18n keys (the "title.settings" placeholder), and a cartoon tank-battle menu backdrop — shipped via #227 (build/CI green); owner eyeball-gate still pending
- [x] Mobile playability + shot-volume (owner asks 2026-06-30) — **on-screen twin-stick touch controls** (`TouchControls` overlay + `TouchInput3DSource`, gated on `DisplayServer.IsTouchscreenAvailable()`): left thumbstick drives, right thumbstick aims, auto-fire while held; wired into the solo (`Arena3DScene`) and networked (`NetArena3DScene`, host + guest) 3D scenes — the APK was previously uncontrollable on a phone (keyboard/mouse only), which blocked both solo and lobby multiplayer. Cannon-shot SFX cut to ~10% (−20 dB on the `Fire` kind). Lobby relay confirmed live (worker mints codes); two-device versus playtest still owner-gated. Pure tests 495✓ + headless GoDotTest 145✓.
- [x] Arcade/mobile fix batch 2026-07-04 (#231/#228/#229/#232) — **web Settings button did nothing**: `SettingsOverlay` was the one Presentation file formatting numbers against ambient culture (`$"{x:F2}"`), which throws on the arcade's ICU-less .NET→WASM runtime and aborted `Build()` — fixed via `SettingsFormat` (InvariantCulture). **Touch Pause button** (solo `Arena3DScene`) + always-visible **Leave button** (versus `NetArena3DScene`, no pause in a live match) so phone players (no Escape key) can reach Settings/Menu/Exit/leave. **Exit → arcade**: new `PlatformExit` port (web → `window.location.href='/'`, desktop → `Quit()`) wired into every Exit button, replacing the browser-inert `GetTree().Quit()`. Tank-death + powerup-pickup SFX **silenced** (removed `SfxPool` dict entries) pending real audio; positional SFX made **audible** (3D distance attenuation disabled); cartoon powerup icons. Headless GoDotTest 149✓ + Infrastructure xUnit 40✓. **Now LIVE in the arcade** — shipped via the P8 WASM re-export + ProjectX #38 (2026-07-04): the web build (`p8/web-export-refresh`) was reconciled onto this batch (the bespoke `#if GODOT_WEB` `ExitGame()` dropped for the runtime `PlatformExit` port), re-exported (healthy 49 MB `.pck`, managed C# compiled in), and deployed to Firebase Hosting; live `index.pck` confirmed at the new size.
- [x] Gameplay owner-asks batch 2026-07-04 (#236/#237/#238) — **powerup name labels** (floating billboarded name over each 3D powerup, reusing `PickupFloater.LabelKeyFor` — EN/ES/DK — so pickups read by name, not just icon); **random Solo map** (`SoloMapSelection.Pick` drops a plain Solo launch into a random one of ALL available maps — built-in arenas + created maps — not always Desert War; corrupt custom map falls back to built-in); **airstrike is a small randomized blob** (`AirstrikeSpread.Grow` Eden growth → ~30-cell organic blob, a different shape each play, down from ~55% of the field); **5 respawns per tank** (`StartingLives` 3→6) with a top-of-screen row of five procedurally-drawn tank icons (`RespawnHud`) dimming as each respawn is spent. Tests: GameLogic 414→420→423, GoDotTest 149→156.
- [x] Leaderboard fixes 2026-07-04 (#239) — victory-screen **Deaths** and **Damage-taken** rankings now sort **ascending** (fewer/less is better, best tank on top) via pure `LeaderboardOrder.Rank`; **Healing** leaderboard renamed **Repairs** (`stats.healing`→`stats.repairs`, EN/ES/DK). **Both batches now LIVE in the arcade** — WASM re-exported and deployed via ProjectX #39 (features) + #40 (leaderboard).
- [~] Arcade-bug + multiplayer-lobby arc 2026-07-05 (`docs/multiplayer-lobby-plan.md`, #243/#244/#245/#246) — **§6b fixed** (#243): fixed-timestep sim (`FixedTimestep` 1/60 s) + swept projectile↔tank hits + zero-alloc hot loop — laggy frames can no longer tunnel shots. **§6c** (#244): touch-web text entry via native browser prompt (`WebTextEntry`) + `CultureSafety` CI scan banning the ICU-less-WASM crash class. **Multiplayer redesign Phases 0–8** (#245 merged, #246 open): Title = Solo | Multiplayer → lobby **browser** (open-games list, Create with mode FFA-default/map Random-default) → 4-seat **room** (gray placeholder cast seeded by lobby code, host-only Start, countdown) → **4-player netcode** (per-slot input attribution stamped by the relay, per-slot snapshot acks, roster-driven host with **AI fill carrying the placeholder names**, solo-start allowed, `LastStanding` round end + winner banner on both roles, map resolved identically by every member — Desert seeded from the lobby code). Custom maps NOT offered online yet (guests lack the file — map-content sync is the follow-up). Remaining: **P9 WASM re-export + ProjectX deploy (owner-gated — nothing is live until then)**; deployed-build click-through; iPad renderer profiling (Compatibility renderer/MSAA/shadows levers); net victory screen; Cliffs teleport pads in the net world.
- [~] Improvement batch 2026-07-11 (feat/improvement-batches, PR open) — five parallel slices, all TDD:
  **combat tuning** (per-ammo damage Missile 3 / Piercing 2 / others 1; new `StatKind.Damage` multiplier;
  Speed/RapidFire boosts now 15 s and REFRESH instead of stacking multiplicatively, via `StatusEffect.Source`;
  shield capped at MaxHp); **bot difficulty** (Easy/Normal/Hard in Settings — AI vision ×0.6/1.0/1.3,
  fire interval ×1.5/1.0/0.85, ±4° aim jitter on Easy — persisted, applied solo + host AI fill);
  **shaped arenas** (`ArenaAuthor.Mask` predicate + Donut Ring / Crossfire / Archipelago, registered in all
  pickers + random pools, EN/ES/DK names; no preview PNGs, same as the other five); **boost director**
  (`PowerupDirector`: seeded 20±5 s timed powerup drops — rarity-weighted, ≥2 cells from tanks/pickups,
  cap 6 live, 30 s despawn — in solo AND net, where pickups now exist for the first time via a
  backward-compatible trailing `Powerups` snapshot section mirrored in both codecs); **online rematch**
  (host-only `rematch` lobby command, phase started→waiting keeping seats/map/mode; Rematch button on the
  net winner banner returns everyone to the room on the same socket). Verified on the integration branch:
  GameLogic 552 + Domain 69 + Infrastructure 54 + Architecture 14 xUnit, worker vitest 86, headless
...
- [~] Parallel batch 2026-07-17 (orchestrated; #261/#262/#263 merged — owner gates below):
...
- [~] Arcade-parity fix batch 2026-07-17 (orchestrated; #264/#265/#266 merged — owner gates below):
  **Randomized min-distance spawns** (#264) — `SpawnTable` rewritten: the mirrored 8-candidate scheme
  (which collapsed centre spawns and nudged duplicates to ADJACENT cells → tanks stuck inside each
  other) replaced by seeded random placement over open non-lethal cells, pairwise Chebyshev separation
  target 10 relaxing to floor 3, farthest-point best-effort below; deterministic per seed
  (`GameSetup.ArenaSeed` / lobby seed); solo + generator + net host rewired; respawns keep their cell.
  **Volcano lava + bridges** (#265) — lava is now an animated 2-layer value-noise ShaderMaterial
  (dark crust, glowing pulsing cracks, world-space, unshaded/no texture fetches — Compatibility/WASM
  safe) replacing the flat yellow emissive plane; every lava crossing widened to 3 cells (test-asserted).
  **Web parity reconcile** (#266) — the 4 June web branches audited commit-by-commit (4 ported, 5
  already-in-main/superseded): main now exports Web directly (csproj/Program.cs/sln + Bootstrap
  `GODOT_WEB` guards; smoke-built 53.7 MB wasm), crypto-free `EntityId` on web (Guid.NewGuid is
  constant under WASM — silently disabled all combat), SfxPool/TranslationLoader `GD.Load` fallbacks,
  deterministic web arena seed, quiet cartoon-pew `fire.ogg` restored with `FireOffsetDb` −20→−6 dB;
  `docs/web-export.md` rewritten; `client/audio/sfx/README.md` audition guide added (all clips
  preview-able + replace-by-name). All batches verified: xUnit 14/69/583/54 + GoDotTest 195 green on
  the final merged tree. Remaining (owner-gated): lava look eyeball, fire-loudness ear-check at −6 dB,
  arcade WASM re-export + ProjectX/Firebase redeploy (confirm before live push), two-device net spawn
  sanity.
