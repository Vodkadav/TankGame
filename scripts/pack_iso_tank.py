"""Stitch the Blender-rendered iso-tank frames into the two committed spritesheets.

scripts/render_iso_tank.py writes hull_NN.png + turret_NN.png into a frames dir. This crops every
frame to ONE shared bounding box (the union of all 32 frames' opaque pixels) so the hull and turret
layers stay pixel-aligned when overlaid in-engine, then lays each layer out as a horizontal strip:

    IsoTankHull.png    = FACINGS frames left-to-right
    IsoTankTurret.png  = FACINGS frames left-to-right

Run with the system Python (Pillow):  py scripts/pack_iso_tank.py <frames_dir> <out_dir>
A composite preview montage is written to the frames dir for eyeballing alignment (not committed).
"""
import sys
...
def load(frames, layer):
    return [Image.open(frames / f"{layer}_{i:02d}.png").convert("RGBA") for i in range(FACINGS)]
...
def main():
    ...
    # preview: hull facing 0 with the turret at four aims, to confirm it stays seated and rotates
