...
| `client/src/Presentation/Arena/GroundSand.png` | Kenney "Top-down Tanks Remastered" (`tileSand1`), resized by `scripts/prep_kenney_ground.py` | CC0 1.0 (public domain, no attribution required) | Source: <https://kenney.nl/assets/top-down-tanks-remastered>. Seamless sand tile resized to the 64 px game tile; tiled across the field by `ArenaScene.BuildGround`, tinted per `ArenaTheme`. |
